...
class ControllerExtensionPaymentDNAPayments extends Controller {
  ...
  public function index() {
    ...
    $is_test_mode = $this->config->get('dna_payments_test_mode') == '1';
    ...
    $terminal = $is_test_mode ? $this->config->get('dna_payments_test_terminal') : $this->config->get('dna_payments_terminal');
    $amount = round((float)$order_info['total'], 2);
    $currency = $order_info['currency_code'];

    $data['isTestMode'] = $is_test_mode;
    $data['isFullRedirect'] = $this->config->get('dna_payments_type') == '1';
    ...
    $response = $this->sendPost($this->getAuthUrl(), array(
      ...
      'client_id' => $is_test_mode ? $this->config->get('dna_payments_test_client_id') : $this->config->get('dna_payments_client_id'),
    ...
    if ($response != null && $response['status'] >= 200 && $response['status'] < 400) {
      ...
      $data['paymentData'] = array(
        ...
        'description' => $this->config->get('dna_payments_description'),
  ...
  public function callback() {
    ...
    if ($response['success']) {
      $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('dna_payments_order_status_paid_id'), '', TRUE);
    } else {
  ...
  private function getClientSecret() {
    $is_test_mode = $this->config->get('dna_payments_test_mode') == '1';
    return $is_test_mode ? $this->config->get('dna_payments_test_client_secret') : $this->config->get('dna_payments_client_secret');
  }
